/**
 * Analysis for AIA-style architectural DXF (Hack_Canada sample).
 *
 * 讀 A-ROOM-BDRY / A-WALL-* / A-DOOR / A-WINDOW / S-COLS / A-BALCONY 等層，
 * 用 point-in-polygon 把「elevator」文字標籤配對到所屬房間。
 *
 * Runtime-neutral — 輸入為 DXF 字串。
 */
...
export function analyzeArchDxf(dxfText: string, source: string, filePath: string) {
  ...
  type Room = { id: number; vertices: Point[]; bbox: Bbox; area: number }
  const rooms: Room[] = []
  let nextId = 0
  for (const e of dxf.entities) {
    ...
    rooms.push({ id: nextId++, vertices: vs, bbox: b, area: (b.maxX - b.minX) * (b.maxY - b.minY) })
  }
  ...
  for (const e of dxf.entities) {
    ...
    if (e.type === 'INSERT' && e.position) {
      inserts.push({ layer, position: { x: e.position.x, y: e.position.y }, name: e.name || '' })
      continue
    }
  ...
  for (const e of dxf.entities) {
    ...
    elevLabels.push({ text: e.text, position: { x: pos.x, y: pos.y }, matchedRoomId: null })
  }
  ...
  return {
    ...
    rooms: rooms.map((r) => ({ id: r.id, vertices: r.vertices, is_elevator: matchedRoomIds.has(r.id) })),
    walls,
    doors,
    windows,
    columns,
