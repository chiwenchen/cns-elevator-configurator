...
**誰在受苦**：一家年產 1000 台新梯的台灣電梯製造商，內部業務團隊每次客戶來電詢價，都得等設計師花 2 天查 CNS 2866、用 AutoCAD 畫設計、確認可行型號，才能回覆客戶一個可信的報價。
...
- 設計流程：設計師用 AutoCAD 手繪 + 人工翻 CNS 2866 查表 + 無自動驗證（使用者確認）
...
> **業務輸入坑道完整規格（見下表），app 輸出 1 到 3 個合法型號推薦，每個含：規格卡、粗估價、CNS 2866 合規章、交期估計。非標特例自動旗標「請聯絡 XX 資深工程師」。**
...
| 坑道深（D） | mm | 內淨深 |
| 坑道總高（H） | mm | 最低樓板到最高樓板 |
| 頂部高度（overhead） | mm | 最高樓板到機房樓板 — **CNS 2866 硬約束** |
| 底坑深度（pit depth） | mm | 最低樓板到坑底 — **CNS 2866 硬約束** |
| 停站數 | 整數 | |
| 額定載重 | kg | 業務跟客戶談好的目標載重 |
| 用途 | 列舉 | 客用 / 貨用 / 病床 / 無障礙 |

**關鍵**：overhead 跟 pit depth 是 CNS 2866 第 X 條（實際條號在第 0 週 spike 時確認）的硬約束，不列出來整個檢核器形同空談。
...
CnsRule = {
  clause_id: string,              // "CNS-2866-5.2.1"
  title: string,
...
**重用**：公司既有產品型錄、歷史 CAD 檔、CNS 2866 內部版、報價 SOP
...
- **Spike 2（半天）**：翻 CNS 2866 PDF，找出跟坑道 / 轎廂尺寸 / overhead / pit depth 相關的**具體條號**，列出實際的 20-30 條核心規則（第一批要數位化的候選）。沒做這一步前，所有關於「CNS 條文」的描述都是虛的。
...
- **D6-8**：把 CNS 2866 Spike 2 挑出的約 30 條 key 成 `CnsRule[]`，每條要能用 validator 跑起來
...
5. **CNS 2866 更新**：條文更新時誰負責同步規則庫？要不要每年例行審核？
...
- [ ] 拿到 20 個歷史 CAD 檔的存取權
- [ ] 拿到 CNS 2866 條文的可用格式（PDF / DOC / 公司內部整理版）
...
2. ✅ **輸入欄位沒定義 overhead / pit depth** — 已在第四節補完整輸入表（8 欄位），並標注這兩項是 CNS 2866 硬約束
